...
HRESULT CreateGraphicsResources(HWND hWnd)
{
	...
	if (pRenderTarget == nullptr)
	{
		...
		if (SUCCEEDED(hr))
		{
			hr = pRenderTarget->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::LightSlateGray), &pCornflowerBlueBrush);
		}
	}
...
int WINAPI WinMain(HINSTANCE hIntance, HINSTANCE hPrevInstance, LPSTR  lpCmdLine, int nCmdShow)
{
	...
	// create the window
	hWnd = CreateWindowEx(0, _T("WindowClass1"), _T("Hello, Engine![Direct 2D]"), WS_OVERLAPPEDWINDOW, 100, 100, 960, 540, NULL, NULL, hIntance, NULL);
